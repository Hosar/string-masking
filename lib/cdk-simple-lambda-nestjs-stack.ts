...
export class CdkLambdaNestjsStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const nestJsLambda = new lambda.Function(this, "CustomerCrudLambda", {
      ...
      handler: "dist/serverless.handler", // Assumes your Lambda code is in index.js
    ...
    const httpApi = new apigatewayv2.HttpApi(this, "NestJsHttpApi", {
      defaultIntegration: new HttpLambdaIntegration(
        "SomeLambdaIntegration",
        nestJsLambda
      ),
    });

    // Define the API Gateway with a custom path
    const customPathApi = new apigatewayv2.HttpApi(
      this,
      "CustomPathNestJsHttpApi"
    );
    ...
    proxyResource.addMethod(
      "ANY",
      new apigateway.LambdaIntegration(nestJsLambda)
    );
